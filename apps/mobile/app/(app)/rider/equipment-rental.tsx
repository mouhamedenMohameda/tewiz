...
export default function EquipmentRentalScreen() {
  ...
  const { data: quote, isError } = useApiQuery<Quote>(
    ['rider', 'quote', 'equipment-rental'],
    '/rider/rides/equipment-rental-quote',
    { staleMs: 5 * 60_000 },
  );
  ...
  if (!quote.enabled) {
    return (
      <Screen>
        <ScreenHeader title="Location Équipement" onBack={() => router.back()} />
      ...
      <View style={{ flex: 1, padding: spacing.lg, justifyContent: 'space-between' }}>
        <View>
          <AppText variant="body" color={colors.ink2} style={{ marginBottom: spacing.lg }}>
            Louer de l'équipement à la journée
          </AppText>
